...
def upd_slots(slots, i, j, ch):
    ...
    new_slots[i][j] = ch
    return tuple((s1, s2) for (s1, s2) in new_slots)
...
def find_slot(slots, hall, i, snail):
    ...
    if not blocked(hall, i, index):
        if slots[index] == (None, None):
            new_hall = upd_hall(hall, [(i, None)])
            new_slots = upd_slots(slots, index, 1, snail)
            dist = abs(i - (index + 1) * 2) + 2
            return (new_slots, new_hall, dist * s_cost[snail])
        elif slots[index] == (None, snail):
            new_hall = upd_hall(hall, [(i, None)])
            new_slots = upd_slots(slots, index, 0, snail)
            dist = abs(i - (index + 1) * 2) + 1
            return (new_slots, new_hall, dist * s_cost[snail])
...
def poss_changes(slots, hall):
    ...
    for i, snail in enumerate(hall):
        if snail is not None:
            ...
            if sl:
                changes.append(sl)
    for i, (s1, s2) in enumerate(slots):
        if s1 is not None:
            changes.extend(find_hall(slots, hall, i, 0, s1))
        elif s2 is not None:
            changes.extend(find_hall(slots, hall, i, 1, s2))
    return changes
    ...
    while q:
        ...
        for ns, nh, nc in poss_changes(slots, hall):
            if (ns, nh) not in passed:
                if nh[0] == 'A':
                    print(ns, nh, nc)
                states[stct] = (ns, nh, cost + nc, past +
                                [(slots, hall, cost)])
...
if __name__ == '__main__':
    print(solve([]))
    #print(h((('A', 'A'), ('B', 'B'), ('C', 'C'), ('D', 'D')), [None] * 11))
