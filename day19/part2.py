...
def man_dist(v1, v2):
    return reduce(operator.add, (abs(v2[i] - v1[i]) for i in range(3)))


def solve(input_list: list):
    scanners = []
    for scanner_str in input_list:
        rbeacons = [[int(n) for n in line.split(',')]
                    for line in scanner_str.split('\n') if not line.startswith('---')]
    ...
    scanners = [v for v in detected.values()]
    mx = -999999
    for i, b in enumerate(scanners):
        for b2 in scanners[i:]:
            d = man_dist(b, b2)
            if d > mx:
                mx = d
    return mx
